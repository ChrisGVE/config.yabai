...
from .fast_data_types import (
    ...
    SingleKey,
    current_fonts,
    glfw_get_system_color_theme,
    num_users,
    opengl_version_string,
    wayland_compositor_data,
)
...
def print_mapping_changes(
    defns: dict[str, str], changes: set[str], text: str, print: Print
) -> None:
    if changes:
        print(title(text))
...
def compare_maps(
    ...
    which = (
        "shortcuts"
        if isinstance(next(iter(initial or final)), Shortcut)
        else "mouse actions"
    )
...
def compare_opts(
    opts: KittyOpts, global_shortcuts: dict[str, SingleKey] | None, print: Print
) -> None:
        ...
        if not kmn and global_shortcuts:
            for action, sk in global_shortcuts.items():
                ...
                if sc not in final:
                    final[sc] = action
        compare_maps(
            final, opts.kitty_mod, initial, default_opts.kitty_mod, print, mode_name=kmn
        )
            ...
            if pid > -1:
                ...
                with suppress(Exception):
                    ...
                    if exe.lower() == "hyprland":
                        raw = (
                            subprocess.check_output(["hyprctl", "version"])
                            .decode()
                            .strip()
                        )
...
def debug_config(
    opts: KittyOpts, global_shortcuts: dict[str, SingleKey] | None = None
) -> str:
    ...
    p(
        f'System color scheme: {green(glfw_get_system_color_theme())}. Applied color theme type: {yellow(theme_colors.applied_theme or "none")}'
    )
