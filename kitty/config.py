#!/usr/bin/env conda run -n kitty python
...
def link(url: str, txt: str) -> str:
    ...
    link_id += 1
    return (
        f"\x1b]8;id={link_id};{url}\x1b\\{txt}\x1b]8;;\x1b\\" if cmdArgs.links else txt
    )
...
def nojust(_: int, s: str):
    return s


def ljust(n: int, s: str):
    return s.ljust(n)


def rjust(n: int, s: str):
    return s.rjust(n)


def center(n: int, s: str):
    return s.center(n)


def formatConf(n: int, s: str):
    return linkConfig(s) + (" " * (n - len(s)))
...
def formatTable(
    a: list[tuple[str]], fmt: list[TabFmt]
        ...
        (
            ...
            "".join(
                list(
                    map(
                        star(
                            lambda val, fmt1, width: (" " * fmt1.indent)
                            + fmt1.fmtFn(fmt1.justFn(width, val))
                        ),
                        zip(row, fmt, field_lens),
                    )
                )
            ),
        )
...
def printTable(
    a: list[list[str]], fmt: list[TabFmt], print: Print, rowFmtFn=lambda row, txt: txt
) -> str:
...
def print_mapping_changes(
    ...
    for k in sorted([(shortcutSortKey(s), s) for s in defns]):
        ...
        flags = (
            "  A  "
            if isadded
            else "  C  " if ischanged else "  -  " if isremoved else "     "
        )  # →
        if cmdArgs.diff and (not isadded and not ischanged and not isremoved):
            continue
        v = formatAction(0, defns[k]) if not isremoved else "     "
        orig = f"  \t({formatAction(0, defns[k])})" if ischanged or isremoved else ""
        if not isremoved:
            action2key.setdefault(re.sub(r" .*$", r"", defns[k]), []).append(
                (k, defns[k])
            )
        if isremoved and not cmdArgs.deleted:
            continue
...
def compare_maps(
    ...
    which = (
        ...
        if what == "keys"
            ...
            "https://sw.kovidgoyal.net/kitty/conf/#mouse-actions", "mouse actions"
        )
    )
    print_mapping_changes(
        what,
        dict(list(ei.items()) + list(ef.items())),
        ei,
        added,
        removed,
        changed,
        f"{cmdArgs.what} {which}:",
        print,
    )
...
def compare_opts(opts: KittyOpts, print: Print) -> None:
    ...
    printConfig(
        link("https://sw.kovidgoyal.net/kitty/conf/", f"{cmdArgs.what} config options:")
    )
    ...
    for f in cmp_opts:
        ...
        if isinstance(val, dict):
            ...
            if isinstance(val, Color):
                ...
                printConfig(
                    dim(f"  \t({str(getattr(defaults, f))})") if ischanged else ""
                )
    ...
    final_s, initial_s = map(
        flatten_sequence_map, (opts.sequence_map, default_opts.sequence_map)
    )
...
def printActions(print: Print):
    ...
    for (group, action, desc), rowTxt in actionsTable:
        ...
        a = re.sub(r" .*$", r"", action)
        if a in action2key:
            if not cmdArgs.empty:
                print(rowTxt)
            a2ksorted = [
                v[1]
                for v in sorted(
                    [((v, shortcutSortKey(k)), (k, v)) for k, v in action2key[a]]
                )
            ]
            ...
            if len(a2ksimple) > 0:
                ...
                print()
            printTable(
                [(*shortcutSplit(k), formatAction(0, v)) for k, v in a2kcomposite],
                [
...
def debug_config(opts: KittyOpts) -> str:
    ...
    if cmdArgs.info:
        ...
        if is_macos:
            ...
            p(
                "  ".join(
                    subprocess.check_output(["sw_vers"]).decode("utf-8").splitlines()
                ).strip()
            )
    ...
    if cmdArgs.env:
        ...
        printTable(
            [
                ...
                if k.startswith("LC_") or k.startswith("XDG_") or k in envVars
            ],
        ...
        p(
            green(
                link(
                    ...
                    f"{cmdArgs.what} available actions:",
                )
            )
        )
        printActions(p)
    ...
    return (
        out.getvalue()
        if not cmdArgs.plain
        else re.sub(r"\x1b[^m]*m", "", out.getvalue())
    )
...
""",
    ...
    parser.add_argument(
        ...
        help="Print only the diff vs defaults",
    )
    parser.add_argument(
        "-a",
        "--all",
        action=argparse.BooleanOptionalAction,
        help="Print all parts (default behavior)",
    )
    parser.add_argument(
        "-i",
        "--info",
        action=argparse.BooleanOptionalAction,
        help="Print common info section",
    )
    parser.add_argument(
        "-c",
        "--config",
        action=argparse.BooleanOptionalAction,
        help="Print regular config options section",
    )
    parser.add_argument(
        "-m",
        "--mouse",
        action=argparse.BooleanOptionalAction,
        help="Print mouse bindings section",
    )
    parser.add_argument(
        "-k",
        "--keys",
        action=argparse.BooleanOptionalAction,
        help="Print keyboard shortcuts section",
    )
    parser.add_argument(
        "-l",
        "--colors",
        action=argparse.BooleanOptionalAction,
        help="Print colors section",
    )
    parser.add_argument(
        "-e",
        "--env",
        action=argparse.BooleanOptionalAction,
        help="Print environment variables section",
    )
    parser.add_argument(
        "-t",
        "--actions",
        action=argparse.BooleanOptionalAction,
        help="Print actions section",
    )
    parser.add_argument(
        "--deleted", action=argparse.BooleanOptionalAction, help="Print deleted keys"
    )
    parser.add_argument(
        "--empty",
        "--unassigned",
        action=argparse.BooleanOptionalAction,
        help="Print unassigned actions",
    )
    ...
    parser.add_argument(
        ...
        "--debug",
        action=argparse.BooleanOptionalAction,
        help='Make output closest to "debug_config"',
    )
    parser.add_argument(
        "--links",
        action=argparse.BooleanOptionalAction,
        help="Use terminal codes for hyperlinks",
    )
    parser.add_argument(
        "--plain",
        "--plaintext",
...
def handle_result(args, answer, target_window_id, boss) -> None:
    ...
    boss.display_scrollback(
        boss.active_window,
        debug_config(get_options()),
        title="Full kitty config",
        report_cursor=False,
    )
